...
set config = config || jsonb_build_object('extract_card_candidates', true, 'max_links', 25),
    parser_version = 'eacea-card-catalogue-v3',
    updated_at = now()
where adapter_key = 'official_catalogue_discovery';

-- Preserve v2 evidence but remove it from the active review queue. The card
-- parser emits exactly one canonical candidate per official catalogue entry.
update public.opportunity_candidates c
set review_state = 'superseded',
    review_notes = 'Superseded automatically by the EACEA card-aware parser.',
    updated_at = now()

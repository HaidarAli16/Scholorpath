-- Migration 015: auto generate rules on publish

create or replace function public.generate_rules_for_published_entity(p_entity_type text, p_entity_id uuid, p_normalized_data jsonb)
returns void
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_country_code text;
begin
  -- a. nationality rule if eligibility_origin_countries exists
  v_countries := p_normalized_data->'eligibility_origin_countries';
  if v_countries is not null and jsonb_array_length(v_countries) > 0 then
    insert into public.atomic_rules (
      ...
      p_entity_type, p_entity_id, 'nationality_req', 'nationality', 'in', 'nationality', v_countries, 'hard', 'Requires nationality from eligible countries', 'published', 1
    ) on conflict (entity_type, entity_id, rule_key, version) do update
    set expected_value = excluded.expected_value,
        severity = excluded.severity,
        state = 'published';
  end if;

  -- b. qualification rule based on degree_level
  v_degree := p_normalized_data->>'degree_level';
  if v_degree is not null then
    declare
      v_expected jsonb;
      v_expl text;
    begin
      if v_degree = 'masters' then
        v_expected := '["bachelors_completed", "masters_in_progress", "masters_completed"]'::jsonb;
        v_expl := 'Requires completed bachelor''s degree for master''s admission';
      elsif v_degree = 'phd' then
        v_expected := '["masters_completed", "phd_in_progress", "phd_completed"]'::jsonb;
        v_expl := 'Requires completed master''s degree for PhD admission';
      elsif v_degree = 'bachelors' then
        v_expected := '["high_school_completed", "bachelors_in_progress", "bachelors_completed"]'::jsonb;
        v_expl := 'Requires high school completion for bachelor''s admission';
      end if;
      ...
      if v_expected is not null then
        insert into public.atomic_rules (
          ...
          p_entity_type, p_entity_id, 'qualification_req', 'academic', 'in', 'qualification', v_expected, 'soft', v_expl, 'published', 1
        ) on conflict (entity_type, entity_id, rule_key, version) do update
        set expected_value = excluded.expected_value, state = 'published';
      end if;
    end;
  end if;

  -- c. information rule about destination
  v_country_code := p_normalized_data->>'country_code';
  if v_country_code is not null then
    insert into public.atomic_rules (
      entity_type, entity_id, rule_key, rule_group, operator, profile_field, expected_value, severity, explanation, state, version
    ) values (
      p_entity_type, p_entity_id, 'destination_info', 'logistics', 'eq', 'destinationPreference', to_jsonb(v_country_code), 'information', 'Located in ' || v_country_code, 'published', 1
    ) on conflict (entity_type, entity_id, rule_key, version) do update
    set expected_value = excluded.expected_value, state = 'published';
  end if;

  -- d. funding type rule
  v_funding := p_normalized_data->>'funding_type';
  if v_funding = 'full' then
    insert into public.atomic_rules (
      entity_type, entity_id, rule_key, rule_group, operator, profile_field, expected_value, severity, explanation, state, version
    ) values (
      p_entity_type, p_entity_id, 'funding_req', 'financial', 'in', 'fundingNeed', '["full", "major"]'::jsonb, 'soft', 'Suitable for high funding needs', 'published', 1
    ) on conflict (entity_type, entity_id, rule_key, version) do update
    set expected_value = excluded.expected_value, state = 'published';
  end if;

  -- e. field family rule
  v_field := p_normalized_data->>'field_family';
  if v_field is not null and v_field != 'general' then
    insert into public.atomic_rules (
      entity_type, entity_id, rule_key, rule_group, operator, profile_field, expected_value, severity, explanation, state, version
    ) values (
      p_entity_type, p_entity_id, 'field_req', 'academic', 'in', 'fieldFamily', jsonb_build_array(v_field), 'soft', 'Preference for ' || v_field || ' field', 'published', 1
    ) on conflict (entity_type, entity_id, rule_key, version) do update
    set expected_value = excluded.expected_value, state = 'published';
  end if;

  -- f. english status information rule
  insert into public.atomic_rules (
    entity_type, entity_id, rule_key, rule_group, operator, profile_field, expected_value, severity, explanation, state, version
  ) values (
    p_entity_type, p_entity_id, 'english_info', 'logistics', 'eq', 'englishStatus', '"completed"'::jsonb, 'information', 'English proficiency test generally required', 'published', 1
  ) on conflict (entity_type, entity_id, rule_key, version) do update
  set expected_value = excluded.expected_value, state = 'published';
...
-- modify publish_opportunity_candidate to call generate_rules_for_published_entity

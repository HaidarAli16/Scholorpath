...
export async function POST(request: Request) {
  ...
  try {
    const persistenceClient = user ? createSupabaseAdminClient() : null;
    if (user && !persistenceClient) return NextResponse.json({ error: "Secure recommendation persistence is not configured." }, { status: 503 });
    const evaluation = await evaluateCatalogue(supabase, profile, { userId: user?.id, assessmentId, persistenceClient });
